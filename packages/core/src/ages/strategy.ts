...
/**
 * Record pressure affects only the chronicler/autoplay policy. A player still
 * owns the Record/Omit/Embellish decision and all three options remain legal.
 */
export function ageRecordFactor(ctx: SimCtx, option: AgeRecordPriority): number {
  return activeStrategies(ctx).some((s) => s.record === option) ? 2 : 1;
}
