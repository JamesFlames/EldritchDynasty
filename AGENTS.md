...
- **Outcomes passing on a coin.** Gate 8 (outcome reach) requires every authored outcome to resolve at least once in a hundred thousand-year runs, and a handful still sit at 2%: `archive_read_back_at_the_assize`'s two branches, `frame_the_copy_that_was_short`, `frame_read_out_in_a_hall_at_cawdry` and `who_gets_the_physician/the_weak_one -> kept_them_both`. Any change to any phase's draws re-rolls them, so that gate can go red on content nobody touched. The Insurrection was the worst case and is done — all four of its scenes now carry one measured weight, with the numbers and the ceiling written at the top of `packages/content/events/age_insurrection.yaml`. The two frame interludes are a different shape and need the Discrepancies they read to be likelier, not a weight.
